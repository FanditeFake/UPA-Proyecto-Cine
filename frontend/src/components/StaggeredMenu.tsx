...
export const StaggeredMenu = ({
  ...
  return (
    ...
    >
      ...
      <header className="staggered-menu-header" aria-label="Encabezado principal">
        <button
          type="button"
          className="sm-logo"
          aria-label={onLogoClick ? `${logoText}, ir al inicio` : "Logo"}
          onClick={() => {
            if (onLogoClick) {
              onLogoClick();
              closeMenu();
      ...
      <aside id="staggered-menu-panel" ref={panelRef} className="staggered-menu-panel" aria-hidden={!open}>
        <div className="sm-panel-inner">
            ...
            {items && items.length ? (
              items.map((it, idx) => (
                <li className="sm-panel-itemWrap" key={it.label + idx}>
                  <a
                    className={`sm-panel-item ${it.active ? "sm-panel-item-active" : ""}`}
                    href={it.link ?? "#"}
          ...
          {displaySocials && socialItems && socialItems.length > 0 && (
            <div className="sm-socials" aria-label="Cuenta">
              ...
              <ul className="sm-socials-list" role="list">
                {socialItems.map((s, i) => (
                  <li key={s.label + i} className="sm-socials-item">
                    <a
                      href={s.link ?? "#"}
                      className={`sm-socials-link ${s.onClick ? "" : "sm-socials-link-static"}`}
                      onClick={(e) => {
                        e.preventDefault();
                        if (s.onClick) {
                          s.onClick();
